  ...
  foreach($_POST as $k=>$v)
    ...
    if ( $ajax == 1) {
        if( !empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['offer']) ) {
            ...
            $mail->Subject = "A new mail from ".$_POST['name']." | portfolio contact form feedback";
            ...
            if(!$mail->Send())
                ...
                {
                    echo '<p id="ok">Email sent! Thank you for contacting me</p>';
                }
        }
        else {
           echo '<p id="error">Please, fill all forms</p>';
            ...
            $mail->AddAddress($emailAddress);
            $mail->SetFrom($_POST['email'], $_POST['name']);
            $mail->Subject = "A new email from ".$_POST['name']." | portfolio contact form feedback";
            ...
            $mail->MsgHTML($msg);
                 ...
                 if(!$mail->Send())
                    ...
                    {
                        echo 'Email sent! Thank you for contacting me';
                    }
           }
           else {
